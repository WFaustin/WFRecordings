...
>
    <div
        class={"fixed bottom-0 w-full w-full duration-200 flex p-10 z-[10] "
    + ( y > 0 ? 'opacity-full pointer-events-auto' : 'opacity-0 pointer-events-none')
    }
    >
        <button on:click={goTop} 
        class='ml-auto rounded-full bg-slate-900 text-violet-400 px-3 sm:px-4 
...
<svelte:window bind:scrollY={y} bind:innerHeight={innerHeight} bind:innerWidth={innerWidth}></svelte:window>